...
module.exports = {
	...
	async execute(message, args, client) {
		...
		async function nextBttvPage() {
			const getNextBttvEmotePage = await fetch(`https://api.betterttv.net/3/emotes/shared?before=${currentCursor}&limit=100`, {
				method: 'GET',
			}).then((res) => res.json());
			...
			for (const emote of getNextBttvEmotePage) {
				const xd = await fetch(`https://api.betterttv.net/3/account/editors/${emote.user.id}`, {
					method: 'PUT',
					headers: {
				...
				console.log(xd);
				nextIndex++;
				console.log(`Added ${xd.name}, ${nextIndex} / ${getNextBttvEmotePage.length}`);
