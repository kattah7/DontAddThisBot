                ...
                if (args[0] == 'main') {
                    if (args[2]) {
                        ...
                        if (isNull.data == null) {
                            ...
                            const isEmoteInThatChannel = isNull.data.emoteSet.emotes.find(
                                (emote) => emote.name === args[1]
                            );
                            if (isEmoteInThatChannel) {
                                ...
                                if (channelEmotes.data.emoteSet.emotes.length == channelEmotes.data.emoteSet.capacity) {
                                    ...
                                    const emoteAlreadyExist = channelEmotes.data.emoteSet.emotes.find(
                                        (emote) => emote.name === args[1]
                                    );
                    ...
                    if (channelEmotes.data.emoteSet.emotes.length == channelEmotes.data.emoteSet.capacity) {
                        ...
                        if (emoteAlreadyExist) {
                            ...
                            if (/https:\/\/(next\.)?7tv\.app\/emotes\/\w{24}/g.test(args[1])) {
                                ...
                                const linkEmoteAlreadyExist = channelEmotes.data.emoteSet.emotes.find(
                            ...
                            if (searchEmotes) {
                                ...
                                const findThatEmote = searchEmotes.data.emotes.items.find(
                                    (emote) => emote.name === args[1]
                                );
                                if (findThatEmote) {
                                    ...
                                    const linkEmoteAlreadyExist = channelEmotes.data.emoteSet.emotes.find(
                                        (emote) => emote.name === notFoundEmote
                                    );
                                    if (linkEmoteAlreadyExist) {
                                        return {
                    ...
                    if (args[2]) {
                        ...
                        if (isNull.data == null) {
                            ...
                            if (isEmoteInThatChannel) {
                                ...
                                const isEmoteAlreadyInChannel = channelEmotes.emote_sets.find(
                                ...
                                if (isEmoteAlreadyInChannel) {
                                    if (isEmoteAlreadyInChannel.emotes.length == isEmoteAlreadyInChannel.capacity) {
                                        ...
                                        const isEmoteAlreadyEnabledInMySet = isEmoteAlreadyInChannel.emotes.find(
                                            (emote) => emote.name === args[1]
                                        );
                    ...
                    if (findThatEmoteSet) {
                        const channelEmotes = await utils.V3ChannelEmotes(channelID);
                        const isChannelEmotesFullInSet = channelEmotes.emote_sets.find(
                            (emote) => emote.name === args[0]
                        );
                        ...
                        if (channelCurrentEmotesAmount == channelTotalCapacity) {
                            ...
                            const emoteAlreadyExist = isChannelEmotesFullInSet.emotes.find(
                                (emote) => emote.name === args[1]
                            );
                            if (emoteAlreadyExist) {
                                return {
                                ...
                                if (/https:\/\/(next\.)?7tv\.app\/emotes\/\w{24}/g.test(args[1])) {
                                    ...
                                    const linkEmoteAlreadyExist = isChannelEmotesFullInSet.emotes.find(
                                        (emote) => emote.name === emoteName.data.emote.name
                                    );
                                    if (linkEmoteAlreadyExist) {
                                        return {
