		...
		if (isBotEditorInChannelEditors) {
			...
			if (isSenderIDEditor || ChannelOwnerEditor) {
				if (args[0] == 'main') {
					...
					const emoteSets = await utils.EmoteSets(
						channelID,
					);
					const findThatEmoteSet = emoteSets.find(
						(emoteSet) =>
							emoteSet.name ===
							args[0],
					);
					if (args[2]) {
						...
						if (isNull.data == null) {
							...
							if (
								...
								const channelEmotes =
									await utils.V3ChannelEmotes(
										channelID,
									);
								const isEmoteAlreadyInChannel =
								...
								if (
									isEmoteAlreadyInChannel
								) {
									if (
										isEmoteAlreadyInChannel
											.emotes
											.length ==
										isEmoteAlreadyInChannel.capacity
									) {
										return {
											text: `⛔ "${args[0]}" emote slots are already full...`,
										};
									} else {
										const isEmoteAlreadyEnabledInMySet =
											isEmoteAlreadyInChannel.emotes.find(
												(
													emote,
												) =>
													emote.name ===
					...
					if (findThatEmoteSet) {
						...
						const channelCurrentEmotesAmount =
							isChannelEmotesFullInSet
								.emotes
								.length;
						const channelTotalCapacity =
							isChannelEmotesFullInSet.capacity;
						if (
							channelCurrentEmotesAmount ==
							channelTotalCapacity
						) {
							return {
