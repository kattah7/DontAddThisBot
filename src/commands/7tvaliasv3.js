...
module.exports = {
    ...
    execute: async (msg, args, client, xd, params) => {
        function findEmote(emote) {
            ...
            if (!findThatEmote) {
                return client.say(msg.channelName, `⛔ Emote "${args[0]}" not found in ${msg.channelName}`);
            }
        ...
        function findEmoteParams(emote) {
            ...
            if (!findParamEmoteSet) {
                return client.say(msg.channelName, `⛔ Emote set "${params.set}" not found`);
            }
            const findThatEmote = findParamEmoteSet.emotes.find((x) => x.name == emote);
            if (!findThatEmote) {
                return client.say(msg.channelName, `⛔ Emote "${args[0]}" not found in set ${params.set}`);
            }
        ...
        async function aliasEmote(emoteID, setID, aliasName) {
            ...
            if (renameEmote.errors) {
                ...
                if (params.set) {
                    return client.say(
                        msg.channelName,
                        `7tvM Emote "${args[0]}" in set ${params.set} has been aliased to "${args[1]}"`
                    );
                } else {
