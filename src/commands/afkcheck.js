...
module.exports = {
    ...
    execute: async (message, args, client) => {
        ...
        let { body: userData, statusCode } = await got(`https://supinic.com/api/bot/afk/check?username=${targetUser}`, {
            timeout: 10000,
        ...
        if (!afkStatus) {
            ...
            const ms = new Date().getTime() - Date.parse(afkStatus.started);
            return {
                text: `${targetUser} went AFK(${afkStatus.status}) ${humanizeDuration(ms)} ago BatChest Reason: ${
                    afkStatus.text
                }`,
            };
        }
