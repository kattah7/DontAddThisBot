...
module.exports = {
	...
	async execute(message, args, client, userdata, params, channelData, cmd) {
		...
		const { rows } = await bot.SQL.query(`SELECT * FROM channel_settings WHERE twitch_id = '${message.channelID}' AND command = '${command}';`);

		if (cmd === 'disable') {
			if (!rows[0] || rows[0].length === 0) {
				await bot.SQL.query(
					`INSERT INTO channel_settings (twitch_id, twitch_login, command, aliases, is_disabled) VALUES ('${message.channelID}', '${message.senderUsername}', '${command}', '${JSON.stringify(
						aliases,
					)}', 1);`,
				);
				...
				await bot.SQL.query(`UPDATE channel_settings SET is_disabled = 1 WHERE twitch_id = '${message.channelID}' AND command = '${command}';`);
			...
			if (rows[0].is_disabled === 1) {
				await bot.SQL.query(`UPDATE channel_settings SET is_disabled = 0 WHERE twitch_id = '${message.channelID}' AND command = '${command}';`);
