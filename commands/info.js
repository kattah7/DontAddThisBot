...
module.exports = {
    ...
    execute: async (message, args, client) => {
        ...
        if (message.senderUsername == await utils.PoroNumberOne()) {
            client.privmsg(message.channelName, `.me ${message.senderUsername}, ${targetUser} is currently equipping  ${userData.badge.length > 0 ? userData.badge[0].title : "No Badge"} BatChest and their preferred language is ${userData.settings.preferredLanguageTag} FeelsDankMan and is ${userData.roles.isAffiliate ? "Affiliate ✅" : "not a Affiliate ❌"} peepoHappy and is ${userData.roles.isPartner ? "Partner ✅" : "not a Partner ❌"} imGlitch and is ${userData.roles.isSiteAdmin ? "Admin ✅" : "not a Admin ❌"} monkaS and is ${userData.roles.isStaff ? "Staff ✅" : "not a Staff ❌"} monkaOMEGA`)
        } else {
            return {
