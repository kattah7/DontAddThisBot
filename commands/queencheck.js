    ...
    execute: async(message, args) => {
        ...
        if (data) {
            if (data.followedAt == null) {
                ...
                if (data.cumulative.months == 0) {
                    return {
                        text: `${data.username} was never subbed to pokimane & following for ${humanizeDuration(followAge)} ThankEgg`
                    } 
                }
            } else if (data.cumulative.months > 0) {
                if (data.subscribed == false) {
                    return {
                        text: `${data.username} was previously subbed to pokimane for ${data.cumulative.months} months & following for ${humanizeDuration(followAge)} ThankEgg`
                    }
                } else if (data.subscribed == true) {
                    return {
