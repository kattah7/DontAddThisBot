        ...
        if (data) {
            if (data.followedAt == null) {
                if (data.cumulative.months == 0) {
                    ...
                    if (message.senderUsername == process.env.NUMBER_ONE) {
                        client.privmsg(message.channelName, `.me ${data.username} is subbed to xQc for ${data.cumulative.months} months & not following. xqcL`)
                    } else {
                        return {
                    ...
                    if (message.senderUsername == process.env.NUMBER_ONE) {
                        client.privmsg(message.channelName, `.me ${data.username} is previously subbed to xQc for ${data.cumulative.months} months & not following. xqcL`)
                    } else {
                        return {
                ...
                if (data.cumulative.months == 0) {
                    if (message.senderUsername == process.env.NUMBER_ONE) {
                        client.privmsg(message.channelName, `.me ${data.username} was never subbed to xQc & following for ${humanizeDuration(followAge)} xqcL`)
                    } else {
                        return {
                ...
                if (data.subscribed == false) {
                    if (message.senderUsername == process.env.NUMBER_ONE) {
                        client.privmsg(message.channelName, `.me ${data.username} was previously subbed to xQc for ${data.cumulative.months} months & following for ${humanizeDuration(followAge)} xqcL`)
                    } else {
                        return {
                    ...
                    if (message.senderUsername == process.env.NUMBER_ONE) {
                        client.privmsg(message.channelName, `.me ${data.username} is subbed to xQc for ${data.cumulative.months} months & following for ${humanizeDuration(followAge)} xqcL`)
                    } else {
                        return {
                ...
                if (message.senderUsername == process.env.NUMBER_ONE) {
                    client.privmsg(message.channelName, `.me ${data.username}'s subscription is hidden, Try hovering over their sub badge. Following for ${humanizeDuration(followAge)} xqcL`)
                } else {
                    return {
