...
module.exports = {
    ...
    execute: async(message, args, client) => {
        ...
        if (today >= 7 && today <= 20) { // The NYSE is open from Monday through Friday 9:30 a.m. to 4:00 p.m. NY Time. 
            ...
            const isNull = await utils.Invest(args[0].toUpperCase())
                ...
                if (isNull == null || isNull.dp == null) {
                    ...
                    if (channelData.poroCount >= args[1]) {
                        await bot.DB.poroCount.updateOne({ id: message.senderUserID },  {$addToSet:{ invest:   [{ username: message.senderUsername, id: message.senderUserID, stock: args[0].toUpperCase(), amount: args[1], investedAt: new Date()  }] }} ).exec();
                        await bot.DB.poroCount.updateOne({ id: message.senderUserID }, { $set: { poroCount: channelData.poroCount - args[1] } }, {multi: true} ).exec();
                        return {
