            ...
            if (data.followedAt == null) {
                if (data.cumulative.months == 0) {
                    ...
                    if (message.senderUsername == (await utils.PoroNumberOne())) {
                        client.privmsg(
                            message.channelName,
                            `.me ${data.username} is subbed to trainwrecks for ${data.cumulative.months} months & not following. 🦍`
                        );
                    } else {
                        return {
                    ...
                    if (message.senderUsername == (await utils.PoroNumberOne())) {
                        client.privmsg(
                            message.channelName,
                            `.me ${data.username} is previously subbed to trainwrecks for ${data.cumulative.months} months & not following. 🦍`
                        );
                    } else {
                        return {
                ...
                if (data.subscribed == false) {
                    if (diffDays < 365) {
                        if (message.senderUsername == (await utils.PoroNumberOne())) {
                            client.privmsg(
                                message.channelName,
                                `.me ${data.username} was previously subbed to trainwrecks for ${
                    ...
                    if (diffDays < 365) {
                        if (message.senderUsername == (await utils.PoroNumberOne())) {
                            client.privmsg(
                                message.channelName,
                                `.me ${data.username} is subbed to trainwrecks for ${
                                    data.cumulative.months
                                } months & following for ${humanizeDuration(followAge)} WutFace`
                            );
                        } else {
