            ...
            if (tc || ChannelOwnerEditor) {
                    ...
                    if (isNull.data == null) {
                        ...
                        if (findEmoteInChannel2) {
                            ...
                            if (findEmoteInChannel) {
                                if (message.senderUsername == (await utils.PoroNumberOne())) {
                                    return client.privmsg(
                                        message.channelName,
                                        `.me ⛔ ${args[0]} is already in ${message.channelName}`
                                    );
                                } else {
                                    return {
                            ...
                            if (availableEmotes == xddddd.data.emoteSet.capacity) {
                                ...
                                if (KEKG != args[0]) {
                                    ...
                                    if (message.senderUsername == (await utils.PoroNumberOne())) {
                                        return client.privmsg(
                                            message.channelName,
                                            `.me 7tvM Successfully added "${args[0]}" to ${message.channelName} from ${args[1]} & auto-aliased to "${args[0]}"`
                                        );
                                    } else {
                                        return {
                                ...
                                if (message.senderUsername == (await utils.PoroNumberOne())) {
                                    return client.privmsg(
                                        message.channelName,
                                        `.me 7tvM Successfully added "${args[0]}" to ${message.channelName} from ${args[1]}`
                                    );
                                } else {
                                    return {
                            ...
                            if (message.senderUsername == (await utils.PoroNumberOne())) {
                                return client.privmsg(
                                    message.channelName,
                                    `.me ⛔ Emote "${args[0]}" is not in ${args[1]}`
                                );
                            } else {
                                return {
                ...
                if (findEmoteInChannel) {
                    if (message.senderUsername == (await utils.PoroNumberOne())) {
                        return client.privmsg(
                            message.channelName,
                            `.me ⛔ Emote "${args[0]}" already exists in ${message.channelName}`
                        );
                    } else {
                        return {
                    ...
                    if (/https:\/\/(next\.)?7tv\.app\/emotes\/\w{24}/g.test(url)) {
                        ...
                        if (findEmoteInChannel2) {
                            if (message.senderUsername == (await utils.PoroNumberOne())) {
                                return client.privmsg(
                                    message.channelName,
                                    `.me ⛔ ${message.channelName} already has ${emoteName.data.emote.name}`
                                );
                            } else {
                                return {
                        ...
                        if (availableEmotes == xddddd.data.emoteSet.capacity) {
                            ...
                            if (message.senderUsername == (await utils.PoroNumberOne())) {
                                return client.privmsg(
                                    message.channelName,
                                    `.me 7tvM Successfully added emote "${emoteName.data.emote.name}" to ${message.channelName}`
                                );
                            } else {
                                return {
                        ...
                        if (findThatEmote) {
                            ...
                            if (availableEmotes == xddddd.data.emoteSet.capacity) {
                                ...
                                if (message.senderUsername == (await utils.PoroNumberOne())) {
                                    return client.privmsg(
                                        message.channelName,
                                        `.me 7tvM Successfully added emote "${args[0]}" to ${message.channelName}`
                                    );
                                } else {
                                    return {
                ...
                if (message.senderUsername == (await utils.PoroNumberOne())) {
                    return client.privmsg(
                        message.channelName,
                        `.me ⛔ You are not a editor in ${message.channelName}, ask the broadcaster to add you as an editor by typing <|editor add ${message.senderUsername}>`
                    );
                } else {
                    return {
