      ...
      const { body: pogger, statusCode2 } = await got.post(`https://gql.twitch.tv/gql`, {
          ...
          if (pogger[0].data) {
              if (pogger[0].data.user == null) {
                ...
                if (message.senderUsername == await utils.PoroNumberOne()) {
                    return client.privmsg(message.channelName, `.me ${targetUser} do not have any followers to display.`)
                }
                return {
                ...
                if (message.senderUsername == await utils.PoroNumberOne()) {
                    return client.privmsg(message.channelName, `.me Seems like ${targetUser} blocked this person :p following since ${DATE.split("T")[0]}`)
                }
                    return {
                ...
                const DATE = pogger[0].data.user.followers.edges[0].followedAt
                if (message.senderUsername == await utils.PoroNumberOne()) {
                    return client.privmsg(message.channelName, `.me ${targetUser} first ever follower, ${NAME} has been following since ${DATE.split("T")[0]}`)
                }
                return {
