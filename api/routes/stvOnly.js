...
router.post('/api/bot/stv', async (req, res) => {
    ...
    if (channel.stvOnly) {
        try {
            await bot.DB.channels.updateOne({ id: id }, { $set: { stvOnly: false } });
        ...
        try {
            await bot.DB.channels.updateOne({ id: id }, { $set: { stvOnly: true } });
            return res.status(200).json({
                success: true,
                message: `${channel.username} is now 7tv cmds only`,
            });
        } catch (err) {
            return res.status(500).json({
                success: false,
